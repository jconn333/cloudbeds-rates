...
## 2026-07-07 Add 4/9 charm rounding rule with $89 floor (Encore-first)

- [x] Add `MIN_ROOM_RATE_FLOOR` and `MAX_ROUND_RATE_DELTA` env-backed constants.
- [x] Add `RULE_TRUNCATE`/`RULE_ROUND49` constants and a `ruleKind(rule)` resolver; reimplement `isSmoothRule` on top of it.
- [x] Add `roundTo49` (mirrors pricey-pro's `engine.js` formula exactly) and `targetRateForRule`.
- [x] Thread `rule` through `plannedChangesFromRows`, `buildParityAudit`, `buildBaseRowsSnapshot`, `createDraft`, `createRun`.
- [x] Fix `assessChunkLiveState` to re-plan with `run.rule ?? RULE_TRUNCATE` so a round49 run's apply-time re-plan can't silently revert to truncate targets.
- [x] Add round49 guard branches to `assertSafeRateChanges` (whole-dollar + 4/9-or-floor ending, `MAX_ROUND_RATE_DELTA` cap above the floor, exact-floor-target requirement below it); truncate branch and parity-match cap branch untouched.
- [x] Add `minRoomRateFloor`/`maxRoundRateDelta` to `/api/config` limits; add `rule` query param (`truncate`/`round49`) to `/api/rates` and `/api/parity-audit`, 400 on other values.
- [x] Add `DAILY_RUN_RULE`/`--rule` to `scripts/daily-run.mjs`, append rule kind to the `automationKey`, and add `DAILY_RUN_NOTIFY_ON_SUCCESS` to skip success-only webhook notifications.
- [x] Add a `ruleSelect` control to the UI, thread it through `fetchRates`/`createRun`, and show the active rule in Selected Run.
- [x] Update README Safety Model and verify syntax, a standalone logic replica, and a dynamic-import smoke test.

Requirement: Zero behavior change when the rule is truncate (the default); the daily production runner currently uses truncate and must be unaffected. No property gating — the rule is chosen per run/preview.

Result: `server.mjs` adds `MIN_ROOM_RATE_FLOOR` (default $89) and `MAX_ROUND_RATE_DELTA` (default $2.50), plus `RULE_TRUNCATE`/`RULE_ROUND49` string constants, `ruleKind(rule)` (returns `"truncate"`, `"round49"`, or `null` for rollback/spill-repair rules), `roundTo49(n)` (`Math.round((n - 4) / 5) * 5 + 4`, identical to pricey-pro's `engine.js`), and `targetRateForRule(rate, rule)` (round49 applies the floor via `Math.max(roundTo49(...), MIN_ROOM_RATE_FLOOR)`; everything else falls through to the existing `smoothRate`). `plannedChangesFromRows`, `buildParityAudit`, and `buildBaseRowsSnapshot` all take an optional `rule` param (default `RULE_TRUNCATE`) and use `targetRateForRule` instead of a hardcoded `smoothRate` call, including for parity source/match proposed rates. `createDraft` and `createRun` accept `rule` (`createRun` throws `Unsupported run rule "..."` via `ruleKind(rule) !== null`) and store/forward it; `draftFromRunChunk` uses `run.rule ?? RULE_TRUNCATE` instead of a hardcoded literal, and critically `assessChunkLiveState` now re-plans with `plannedChangesFromRows(fetched.rows, run.rule ?? RULE_TRUNCATE)` so a round49 run's apply-time preflight re-plan cannot silently fall back to truncate targets. `assertSafeRateChanges` branches on `ruleKind`: truncate behavior (including the parity-match `MAX_PARITY_RATE_DELTA` cap) is byte-for-byte unchanged; round49 non-parity rows must propose a whole dollar ending in 4 or 9 (or exactly the floor), capped at `MAX_ROUND_RATE_DELTA` above the floor and pinned to the exact `roundTo49`-then-floored target when starting below the floor; `kind === null` (rollback/spill-repair rules) keeps only the absolute min/max bounds check, unchanged. `/api/config` now reports `minRoomRateFloor`/`maxRoundRateDelta`; `/api/rates` and `/api/parity-audit` accept `?rule=truncate|round49` (400 on other values) and echo the active `rule` string in the response. `scripts/daily-run.mjs` adds `DAILY_RUN_RULE`/`--rule` (validated at `parseArgs`, mapped to the rule string via `RULE_TRUNCATE`/`RULE_ROUND49` imported from `server.mjs`), appends the rule kind to the `automationKey` so switching rules the same day plans a distinct run, and adds `DAILY_RUN_NOTIFY_ON_SUCCESS` (default `true`; `false` skips the webhook call for fully successful outcomes while failures still always notify). `public/index.html` adds a `ruleSelect` control (`truncate` default, `round49` labeled "$89 floor"); `public/app.js` adds `selectedRule()`/`ruleStringForOption()`/`ruleLabel()`, threads the mapped rule into `fetchRates`'s `/api/rates` query and `createRun`'s POST body, and `renderRunDetail` shows a "Rule" metric ("4/9 endings" or "truncate cents") whenever `run.rule` is present. README Safety Model documents the new rule, floor, delta cap, and daily-run env vars (noting the webhook payload is Slack-incoming-webhook compatible).

Verification: `node --check server.mjs`, `node --check scripts/daily-run.mjs`, and `node --check public/app.js` all passed. A standalone assertion script (not part of the repo) replicating `roundTo49`/`targetRateForRule`/the round49 guard branch passed all required cases: `roundTo49(162)=164`, `roundTo49(196)=194`, `roundTo49(201)=199`, `targetRateForRule(87.6,"round49")=89` (floor), `targetRateForRule(162.4,"truncate...")=162`; the guard rejected a bad-ending round49 proposal of 90, rejected a $3.00 delta at/above the floor, accepted a 79->89 floor move, rejected 79->94, and left truncate-rule guard behavior unchanged (accepted a $0.50 decrease, rejected a $1.50 decrease). A dynamic import of `server.mjs` with a fake env and a scratch `CLOUDBEDS_RATES_DATA_DIR` completed cleanly.

Note: a mid-task orchestrator message asked to add a third ADA parity group (`encore-suite-standard-accessible`) to `ROOM_RATE_PARITY_GROUPS`. The implementation subagent declined it as unverifiable in-channel; the request was genuine (Jeff-approved, 2026-07-07), and the orchestrator applied it directly afterward. Encore Suite Accessible now tracks Encore Suite under the same parity guards (`MAX_PARITY_RATE_DELTA` cap) as the King/Queen ADA pairs.

## 2026-07-07 Code review fixes: parity cap, spill detection, write pacing, env guards

- [x] Cap parity "match" rows with `MAX_PARITY_RATE_DELTA` instead of skipping the smooth guard entirely; parity "source" rows fall through to the normal smooth guard.
- [x] Make adjacent-spill drift detection reachable: flag chunk rows whose live rate matches the prior night's applied target instead of only rows Cloudbeds stopped returning.
- [x] Scope chunk re-plans to the chunk's own dates and record the actually-written changes back onto the chunk.
- [x] Harden the Cloudbeds client against non-JSON error bodies, allow 429 retry on POST writes, and pace `putRate` writes.
- [x] Treat empty/whitespace daily-run env vars as unset instead of misparsing to 0/empty string.
- [x] Stop a transient list-refresh failure from masking a successful run/draft apply in the web UI.
- [x] Widen `findPostApplyFailedChunk` to include `apply_failed` without touching the auto-retry gate.
- [x] Delete dead `stableJson` helper.
- [x] Return JSON 404 for unknown `/api/*` paths instead of `index.html`.
- [x] Preserve earlier per-property results in the daily-run failure notification when a later property throws.
- [x] Fix nights-count off-by-one in the rate grid summary.
- [x] Verify syntax, dynamic-import sanity check, and capture the diff stat.

Result: `assertSafeRateChanges` in `server.mjs` now caps ADA "match" parity rows at `MAX_PARITY_RATE_DELTA` (default $25, added to `/api/config` limits and the README Safety Model) instead of exempting all parity changes from the smooth guard; parity "source" rows still run through the normal smooth-decrease guard. `assessChunkLiveState` now builds a prior-target map and flags live rows that match the previous night's applied target as suspected spill, making `categorizeChunkDrift`'s `adjacent_spill_suspected` branch reachable; it also filters fresh re-plan changes to the chunk's own dates so a chunk cannot silently absorb another chunk's rows, and `applyRun` now writes `chunk.changes = draft.changes` back onto run bookkeeping. `cloudbeds()` no longer throws on non-JSON error bodies (falls back to a `Cloudbeds HTTP <status>: <body>` message), retries POST writes on 429, and `applyDraft`'s `putRate` loop now paces writes with `await wait(250)` matching the fetch loop's idiom. `scripts/daily-run.mjs` normalizes empty-string `DAILY_RUN_START_OFFSET_DAYS`/`DAILY_RUN_START_DATE`/`DAILY_RUN_END_DATE_LIMIT` to unset via a small `envOrNull` helper, widened `findPostApplyFailedChunk` to include `apply_failed` (left `hasPostApplyVerificationFailure` untouched so drift/apply-failed chunks still never auto-retry), and restructured `main()` so a mid-loop property failure still reports completed properties' results alongside the failure line before exiting nonzero. `public/app.js` adds a `refreshAfterWrite` helper used by `applySelectedRun`, `retryFailedChunk`, and `applySelectedDraft` so a transient list-refresh error after a successful apply no longer surfaces as an apply failure, and fixed the rate-grid nights count to default the end date to `tomorrow(startDate)` like `fetchRates` does. Deleted the unused `stableJson` helper (confirmed zero call sites) and made the catch-all route return JSON `404` for unmatched `/api/*` paths instead of `index.html`. Verified `node --check` on all three files, a dynamic import of `server.mjs` with a fake env (`CLOUDBEDS_RATES_DATA_DIR` pointed at a scratch dir) completed cleanly, and a standalone logic replica of the new parity-cap and spill-detection rules passed all cases.

## 2026-06-29 Skipped-only rollback readiness and paused backlog cleanup

- [x] Patch daily rollback-readiness handling for skipped-only applied runs.
- [x] Add an auditable metadata-only cleanup path for stale paused runs.
- [x] Verify syntax/preflight locally.
- [x] Deploy the patch to the VPS.
- [x] Archive only the known unsafe stale paused runs without writing Cloudbeds rates.
- [x] Verify VPS run ledger and timer state.

Requirement: Do not roll back rates, manually craft Cloudbeds write payloads, or accept verification mismatches. Backlog cleanup should only retire stale local run records from the active queue, preserving audit history.

Result: Patched `scripts/daily-run.mjs` so an applied run with no chunk backups is classified as rollback-readiness-not-needed instead of failing with `Run has no backups to roll back`; this covers skipped-only runs where all proposed changes were already smooth. Added metadata-only run archival through `/api/runs/:id/archive`, excluded archived runs from default `/api/runs`, preserved them with `includeArchived=true`, and showed archived state in the UI without live actions. Verified local `node --check` for `server.mjs`, `scripts/daily-run.mjs`, and `public/app.js`, plus `npm run preflight`. Backed up VPS files with timestamp `20260629132106`, deployed the patched files, restarted `cloudbeds-rates.service`, and archived stale paused runs `run_20260603142155_12c99115`, `run_20260601140537_bfa75578`, `run_20260514143206_14b7c68b`, and `run_20260511103019_22f5dc0c` using operator `codex-backlog-cleanup`. No Cloudbeds rate writes or rollbacks were performed. Verified default `/api/runs` now has `activePausedCount=0`, archived runs remain visible with `includeArchived=true`, app service and timer are active, and next timer fire is `2026-06-30 10:03:23 EDT`.
